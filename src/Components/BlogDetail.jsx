...
const BlogDetail = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const blogData = await axios.get(`https://blogify-backend.codewithabhinav.in/api/v1/blog/fetchblog/${id}`);
            if(user.id !== undefined){
                const userCommentData = await axios.post("https://blogify-backend.codewithabhinav.in/api/v1/comment/fetchusercomment", {
                postId: id,
                userId: user.id
            });
    ...
    const addCommentHandler = async () => {
        if(token === null){
            ...
            try {
                ...
                const res = await axios.post("https://blogify-backend.codewithabhinav.in/api/v1/comment/addcomment", {
                    content: commentData,
                    postId: id,
                    userId: user.id,
    ...
    const deleteCommentHandler = async (id) => {
        try {
            ...
            const res = await axios.post("https://blogify-backend.codewithabhinav.in/api/v1/comment/deletecomment", { id });
            fetchData();
            toast.dismiss();
